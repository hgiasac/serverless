...
class AwsProvider {
  ...
  getApiGatewayRestApiId() {
    if (this.serverless.service.provider.apiGatewayRestApiId) {
      return this.serverless.service.provider.apiGatewayRestApiId;
    }
    return { Ref: this.naming.getRestApiLogicalId() };
  }

  getApiGatewayRestApiResourceId() {
    if (this.serverless.service.provider.apiGatewayRestApiResourceId) {
      return this.serverless.service.provider.apiGatewayRestApiResourceId;
    }
    return { 'Fn::GetAtt': [this.naming.getRestApiLogicalId(), 'RootResourceId'] };
  }

  apiGatewayResourceExists() {
    // should only run when using shared api
    const params = {
      restApiId: this.getApiGatewayRestApiId(),
    };
    return this.request('APIGateway', 'getResources', params).then(data => {
      console.log(data)
    });
  }
}
